#!/usr/bin/env python3
"""Paginate POOL_MAIN backwards from current to collect May 3 first-hour sigs."""
...
HOUR1_START = int(datetime.datetime(2026,5,3,19,50,0).timestamp())
HOUR1_END   = int(datetime.datetime(2026,5,3,21,0,0).timestamp())
    ...
    for att in range(retries):
        try:
            ...
            if r.status_code == 429:
                w = min(8*2**att, 120); time.sleep(w); continue
            r.raise_for_status()
            d = r.json()
...
def parse_batch(sigs, retries=4):
    for att in range(retries):
        try:
            ...
            if r.status_code == 429:
                w = min(15*2**att, 120); time.sleep(w); continue
            r.raise_for_status()
            time.sleep(0.5)
            d = r.json()
...
print("Paginating POOL_MAIN backwards from current to reach May 3 19:50 UTC...")
print(f"Target: {ts(HOUR1_START)} → {ts(HOUR1_END)}\n")
    ...
    for s in batch:
        ...
        if HOUR1_START <= bt <= HOUR1_END and not s.get("err") and sig:
            if sig not in all_sigs:
                all_sigs[sig] = {"blockTime": bt, "slot": s.get("slot",0)}
                in_win += 1

    print(f"  P{page}: {len(batch)} sigs | {ts(oldest_bt)}→{ts(newest_bt)} | +{in_win} in window (total={len(all_sigs)})")
    before = batch[-1]["signature"]

    # Stop once we've passed the window
    if oldest_bt < HOUR1_START:
        print(f"  Passed target window, stopping")
        break
    if len(batch) < 1000:
        print(f"  Short batch, stopping")
        break

sigs_file.write_text(json.dumps(all_sigs, indent=2))
print(f"\nSaved {len(all_sigs)} sigs in window")

# ─── Parse ────────────────────────────────────────────────────────────────

print(f"\nParsing {len(all_sigs)} sigs...")
all_trades = []
...
for i in range(0, len(sig_list), 100):
    ...
    for tx in parsed:
        ...
        if t:
            all_trades.append(t)
            n += 1
    print(f"  Batch {i//100+1}/{(len(sig_list)+99)//100}: parsed={len(parsed)} trades={n}")

(WORKDIR/"first_hour_trades.json").write_text(json.dumps(all_trades, indent=2))
...
print(f"\n{'='*70}")
print(f"FIRST HOUR  ({ts(HOUR1_START)} → {ts(HOUR1_END)} UTC)")
print(f"{'='*70}")
print(f"Trades: {len(all_trades)} | buys={len(buys)} | sells={len(sells)}")
print(f"SOL in: {sum(t['sol_cost'] for t in buys):.2f} | SOL out: {sum(t['sol_cost'] for t in sells):.2f}")

print(f"\n{'=FIRST 20 BUYS=':^70}")
print(f"  {'Time':8}  {'BELIEVE':>12}  {'SOL':>8}  Wallet")
print("-"*80)
for t in buys[:20]:
...
print(f"  {'First Buy':8}  {'Count':>5}  {'BELIEVE':>12}  {'SOL':>8}  Wallet")
print("-"*85)
...
buyer_list = [{"wallet":w,"first_dt":v["first_dt"],"first_ts":int(v["first_ts"]),
               "buy_count":v["count"],"believe_bought":round(v["believe"],2),"sol_spent":round(v["sol"],4)}
              for w,v in buyers_sorted]
(WORKDIR/"first_hour_buyers.json").write_text(json.dumps(buyer_list, indent=2))
print(f"\nSaved {len(buyer_list)} unique buyers")
